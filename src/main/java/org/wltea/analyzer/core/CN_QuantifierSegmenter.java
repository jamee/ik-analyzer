...
class CN_QuantifierSegmenter implements ISegmenter {
    ...
    private void processCount(AnalyzeContext context) {
        ...
        if (CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()) {
                ...
                for (Hit hit : tmpArray) {
                    ...
                    if (hit.isMatch()) {
                        ...
                        Lexeme newLexeme = new Lexeme(
                                context.getBufferOffset(), hit.getBegin(),
            ...
            // *********************************
            // 对当前指针位置的字符进行单字匹配
            Hit singleCharHit = Dictionary.getSingleton()
                    .matchInQuantifierDict(context.getSegmentBuff(),
                            context.getCursor(), 1);
            if (singleCharHit.isMatch()) {// 首字成量词词
                // 输出当前的词
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(),
                        context.getCursor(), 1, Lexeme.TYPE_COUNT);
                context.addLexeme(newLexeme);
    ...
    private boolean needCountScan(AnalyzeContext context) {
        if ((nStart != -1 && nEnd != -1) || !countHits.isEmpty()) {
            ...
            if (!context.getOrgLexemes().isEmpty()) {
                ...
                if (Lexeme.TYPE_CNUM == l.getLexemeType()
                        || Lexeme.TYPE_ARABIC == l.getLexemeType()) {
    ...
    private void outputNumLexeme(AnalyzeContext context) {
        if (nStart > -1 && nEnd > -1) {
            // 输出数词
            Lexeme newLexeme = new Lexeme(context.getBufferOffset(), nStart,
                    nEnd - nStart + 1, Lexeme.TYPE_CNUM);
            context.addLexeme(newLexeme);
