...
public class IKQueryExpressionParser {
        ...
        for (int i = 0; i < this.elements.size(); i++) {
            ...
            if ('F' == e.type) {
                ...
                if ('\'' == e3.type) {
                    i += 2;
                    if ('=' == e2.type) {
                        TermQuery tQuery = new TermQuery(new Term(e.toString(),
                                e3.toString()));
                        this.querys.push(tQuery);
                    } else if (':' == e2.type) {
                        String keyword = e3.toString();
                        // SWMCQuery Here
                        Query _SWMCQuery = SWMCQueryBuilder.create(
                                e.toString(), keyword, quickMode);
    ...
    private Query toBooleanQuery(Element op) {
        ...
        if ('&' == op.type) {
            if (q1 != null) {
                if (q1 instanceof BooleanQuery) {
                    ...
                    if (clauses.size() > 0
                        ...
                        BooleanClause c = new BooleanClause(q1, Occur.MUST);
                        resultQuery = builder.add(c).build();
                    }
            ...
            if (q2 != null) {
                if (q2 instanceof BooleanQuery) {
                    ...
                    if (clauses.size() > 0
                        ...
                        BooleanClause c = new BooleanClause(q2, Occur.MUST);
                        resultQuery = builder.add(c).build();
                    }
            ...
            if (q1 != null) {
                if (q1 instanceof BooleanQuery) {
                    ...
                    if (clauses.size() > 0
                        ...
                        BooleanClause c = new BooleanClause(q1, Occur.SHOULD);
                        resultQuery = builder.add(c).build();
            ...
            if (q2 != null) {
                if (q2 instanceof BooleanQuery) {
                    ...
                    if (clauses.size() > 0
                        ...
                        BooleanClause c = new BooleanClause(q2, Occur.SHOULD);
                        resultQuery = builder.add(c).build();
                    }
                } else {
            ...
            BooleanClause c = new BooleanClause(q2, Occur.MUST_NOT);
            resultQuery = builder.add(c).build();
        }
        return resultQuery;
    }
     ...
     * @return
     */
    private TermRangeQuery toTermRangeQuery(Element fieldNameEle,
        ...
        return new TermRangeQuery(fieldNameEle.toString(), new BytesRef(
                firstValue), new BytesRef(lastValue), includeFirst, includeLast);
    }
